...
use zbus::zvariant::{OwnedObjectPath, OwnedValue, ObjectPath, Value};
...
impl NmdbusTdmBackend {
    #[inline]
    fn ov<'a, V>(v: V) -> OwnedValue where V: Into<Value<'a>> {
    ...
    async fn scan_internal(&self) -> Result<Vec<Network>> {
        ...
        let msg = nm.call_method("GetDevices", &()).await
            .map_err(|e| Error::CommandFailed(format!("GetDevices call failed: {}", e)))?;
        let devices: Vec<OwnedObjectPath> = msg.body().deserialize()
        ...
        for dpath in devices {
            let dev = Proxy::new(&conn, NM_SERVICE, dpath.as_ref(), "org.freedesktop.NetworkManager.Device").await
                .map_err(|e| Error::CommandFailed(format!("Device proxy error: {}", e)))?;
            let dtype: u32 = dev.get_property::<u32>("DeviceType").await
                .map_err(|e| Error::CommandFailed(format!("Get DeviceType failed: {}", e)))?;
            if dtype != 2 { continue; }
            let ifname: String = dev.get_property::<String>("Interface").await
                .map_err(|e| Error::CommandFailed(format!("Get Interface failed: {}", e)))?;
            if ifname == IFACE_NAME { chosen = Some(dpath.clone()); break; }
            if chosen.is_none() { chosen = Some(dpath.clone()); }
        }
        let dpath = chosen.ok_or_else(|| Error::CommandFailed("No wireless device found".into()))?;

        // Wireless-specific proxy
        let wifi = Proxy::new(&conn, NM_SERVICE, dpath.as_ref(), "org.freedesktop.NetworkManager.Device.Wireless").await
        ...
        let _ = wifi.call_method("RequestScan", &(opts)).await
        ...
        let msg = wifi.call_method("GetAccessPoints", &()).await
            .map_err(|e| Error::CommandFailed(format!("GetAccessPoints failed: {}", e)))?;
        let aps: Vec<OwnedObjectPath> = msg.body().deserialize()
            .map_err(|e| Error::CommandFailed(format!("Decode AccessPoints failed: {}", e)))?;

        // Read properties from each AP
        let mut networks = Vec::new();
        for ap_path in aps {
            let ap = Proxy::new(&conn, NM_SERVICE, ap_path.as_ref(), "org.freedesktop.NetworkManager.AccessPoint").await
                .map_err(|e| Error::CommandFailed(format!("AP proxy error: {}", e)))?;
            let ssid_bytes: Vec<u8> = ap.get_property::<Vec<u8>>("Ssid").await
                .map_err(|e| Error::CommandFailed(format!("Get Ssid failed: {}", e)))?;
            let strength: u8 = ap.get_property::<u8>("Strength").await
            ...
            let security = if rsn != 0 { "WPA2" } else if wpa != 0 { "WPA" } else { "Open" }.to_string();
            let ssid = String::from_utf8(ssid_bytes.clone()).unwrap_or_else(|_| {
                // fallback: hex encode if non-utf8
                format!("{:X?}", ssid_bytes)
            });
            networks.push(Network { ssid, signal: strength, security });
        }
        Ok(networks)
    }
    ...
    async fn get_wifi_device_path(&self) -> Result<OwnedObjectPath> {
        ...
        for dpath in devices {
            ...
            let ifname: String = dev.get_property::<String>("Interface").await
                .map_err(|e| Error::CommandFailed(format!("Get Interface failed: {}", e)))?;
            if ifname == IFACE_NAME { return Ok(dpath); }
            if chosen.is_none() { chosen = Some(dpath); }
        }
    ...
    s_wifi.insert("ssid".into(), Self::ov(self.ap_config.ssid.as_bytes().to_vec()));
    ...
    s_ipv6.insert("method".into(), Self::ov("ignore"));
        ...
        let specific = ObjectPath::try_from("/").map_err(|e| Error::CommandFailed(format!("Invalid object path: {}", e)))?;
        let reply = nm.call_method("AddAndActivateConnection", &(settings, device_path.as_ref(), specific.as_ref())).await
            .map_err(|e| Error::CommandFailed(format!("AddAndActivateConnection failed: {}", e)))?;
        let (con_path, ac_path, _dev_path): (OwnedObjectPath, OwnedObjectPath, OwnedObjectPath) = reply.body().deserialize()
    ...
    async fn stop_ap(&self) -> Result<()> {
        ...
        if let Some(ac_path) = self.active_ap_ac.lock().await.take() {
            let _ = nm.call_method("DeactivateConnection", &(ac_path.as_ref(),)).await;
        }
        if let Some(con_path) = self.active_ap_con.lock().await.take() {
            // delete the connection profile to avoid leftover
            let con = Proxy::new(&conn, NM_SERVICE, con_path.as_ref(), "org.freedesktop.NetworkManager.Settings.Connection").await
    ...
    s_ipv6.insert("method".into(), Self::ov("ignore"));
        ...
        settings.insert("connection".into(), s_connection);
        settings.insert("802-11-wireless".into(), s_wifi);
        if !s_sec.is_empty() { settings.insert("802-11-wireless-security".into(), s_sec); }
        settings.insert("ipv4".into(), s_ipv4);
        settings.insert("ipv6".into(), s_ipv6);
        ...
        let reply = nm.call_method("AddAndActivateConnection", &(settings, device_path.as_ref(), specific.as_ref())).await
            .map_err(|e| Error::CommandFailed(format!("AddAndActivateConnection failed: {}", e)))?;
        let (_con_path, _ac_path, _dev_path): (OwnedObjectPath, OwnedObjectPath, OwnedObjectPath) = reply.body().deserialize()
            .map_err(|e| Error::CommandFailed(format!("AddAndActivate decode failed: {}", e)))?;
        Ok(())
    }
}
...
impl PolicyCheck for NmdbusTdmBackend {
    async fn is_connected(&self) -> Result<bool> {
        ...
        match tokio::process::Command::new("nmcli").arg("-t").arg("-f").arg("STATE").arg("general").output().await {
            Ok(out) => {
